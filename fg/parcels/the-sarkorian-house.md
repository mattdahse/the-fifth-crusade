# Treasure: The Sarkorian House

<!-- id: sarkorian_house -->

## Coin

- 22 GP
- 140 SP
- 310 CP

## Items

### Hooked Labyrinth Token
<!-- count: 3 -->
<!-- type: Gear -->
<!-- cost: 1 gp -->
<!-- weight: 0.5 -->
<!-- nonid: A hooked iron disc, stamped with a maze -->

Iron, stamped with the Ivory Labyrinth's maze and pierced for a thong. Worthless as metal.
Carried openly into the wrong tavern in Drezen it will start a fight; carried quietly to
**Mira Thistledance** it is worth a good deal more than a gold piece, because she is keeping
a count of how many of these are moving through the Marchlands and who is wearing them.

### Slate-Cutter's Maul
<!-- count: 1 -->
<!-- type: Weapon -->
<!-- cost: 12 gp -->
<!-- weight: 10 -->

A roofer's tool, not a weapon, and older than the occupation — Sarkorian work, with the
maker's mark still legible under seventy years of rust. Functions as a masterwork club in
a pinch. **Tobias Runt** would want to see it, and would say so at length.

### Waystation Ledger
<!-- count: 1 -->
<!-- type: Gear -->
<!-- cost: 0 -->
<!-- nonid: A water-swollen book of tallies -->

Three months of tallies in a clerk's hand: how many passed through, going which way,
carrying what. The last four entries are in a different hand and count crusader patrols.
Somebody was using this house to watch the road long before the party arrived.
